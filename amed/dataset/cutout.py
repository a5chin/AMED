...
class CutOuter:
    ...
    def train_test_split(self):
        ...
        data = {t: {"info": info, "licenses": licenses, "images": {}, "annotations": {}, "categories": categories} for t in typ}
        images = [image for image in self.data["images"]]
        annotations = [annotations for annotations in self.data["annotations"]]

        images_train, images_test, annotations_train, annotations_test = train_test_split(images, annotations, test_size=0.3)
        data[typ[0]]["images"], data[typ[0]]["annotations"] = sorted(images_train, key=lambda x: x["id"]), sorted(
            annotations_train, key=lambda x: x["id"]
        )
        data[typ[1]]["images"], data[typ[1]]["annotations"] = sorted(images_test, key=lambda x: x["id"]), sorted(
            annotations_test, key=lambda x: x["id"]
        )
