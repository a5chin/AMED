...
class CutOuter:
    ...
    def cut_out(self):
        ...
        for image, annotation in tqdm(
            zip(self.data["images"], self.data["annotations"])
        ):
            ...
            img.crop((bbox[0], bbox[1], bbox[0] + bbox[2], bbox[1] + bbox[3])).save(
                root / "images" / file_name
            )
            self.data["images"][_id]["height"], self.data["images"][_id]["width"] = (
                bbox[2],
                bbox[3],
            )
    ...
    def train_test_split(self):
        ...
        data[typ[0]]["images"], data[typ[0]]["annotations"] = sorted(
            images_train, key=lambda x: x["id"]
        ), sorted(annotations_train, key=lambda x: x["id"])
        data[typ[1]]["images"], data[typ[1]]["annotations"] = sorted(
            images_test, key=lambda x: x["id"]
        ), sorted(annotations_test, key=lambda x: x["id"])
